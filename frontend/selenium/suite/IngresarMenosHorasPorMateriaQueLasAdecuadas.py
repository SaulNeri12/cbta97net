...
class IngresarMenosHorasPorMateriaQueLasAdecuadas(unittest.TestCase):
    def setUp(self):
    ...
    def test_ingresar_menos_horas_por_materia_que_las_adecuadas(self):
        driver = self.driver
        driver.get("http://127.0.0.1:5500/CrearGrupo/CrearNuevoGrupo.html")
        driver.find_element_by_xpath("(.//*[normalize-space(text()) and normalize-space(.)='Crear Nuevo Grupo'])[2]/following::div[2]").click()
        driver.find_element_by_id("group-letter").click()
        driver.find_element_by_id("group-letter").clear()
        driver.find_element_by_id("group-letter").send_keys("F")
        driver.find_element_by_id("group-name").click()
        driver.find_element_by_id("group-name").clear()
        driver.find_element_by_id("group-name").send_keys("F")
        ...
        driver.find_element_by_xpath("//div[@id='addClassModal']/div/div[2]/div[2]/div").click()
        driver.find_element_by_id("horario-fin").click()
        Select(driver.find_element_by_id("horario-fin")).select_by_visible_text("8:00")
        driver.find_element_by_xpath("//div[@id='addClassModal']/div/div[2]/div[3]/label").click()
        ...
        self.assertEqual(u"Las horas asignadas (1) son menores al mínimo requerido (5) para la materia Matemáticas I", self.close_alert_and_get_its_text())
